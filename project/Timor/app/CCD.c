...
signed char FindMidLine(unsigned char *Arr, signed char *MidPoint, short LeftPoint, short RightPoint);
...
unsigned char CCDM_Arr[256] = { 0 };
unsigned char CCDS_Arr[256] = { 0 };
...
void CCD_GetLine_Main(void)
{
	...
	else
	{
		...
		if (CCDMain_Status.SearchBegin > 108)
		{
			CCDMain_Status.SearchBegin = 108;
		}
		else if (CCDMain_Status.SearchBegin < 20)
		{
...
signed char FindMidLine(unsigned char *Arr, signed char *MidPoint, short LeftPoint, short RightPoint)
{
	...
	for (i = LeftPoint; i < RightPoint; i++)
	{
		...
		Sum2 = Arr[i - 5];
		Sum3 = Arr[i + 5];
		if (((Sum2 - Sum1) > Threshold) && ((Sum3 - Sum1) > Threshold) && ((Sum3 - Sum2<30) && (Sum3 - Sum2>-30)))
		{
...
{
	static float LastControlValue[5];
	static uint8 LostFlagArr_Left[20], LostFlagArr_Right[20];
	static uint8 Cnt_Point = 0, Cnt_ControlValue = 0;
	static uint8 Flag_Cross = 0;
	static int Cnt_Cross = 0, Cnt_ExitCross = 1;
	int i = 0, j = 0, k = 0;


	for (i = 0; i < 4; i++)
	{
		LastControlValue[i] = LastControlValue[i + 1];
	}
	for (i = 0; i < 19; i++)
	{
		LostFlagArr_Left[i] = LostFlagArr_Left[i + 1];
		LostFlagArr_Right[i] = LostFlagArr_Right[i + 1];
	}
	LostFlagArr_Right[19] = CCDMain_Status.Right_LostFlag;
	LostFlagArr_Left[19] = CCDMain_Status.Left_LostFlag;
	LastControlValue[4] = Dir_PID.ControlValue;
	Cnt_Point++;
	Cnt_ControlValue++;
	if (Cnt_ControlValue == 5)
	{
		Cnt_ControlValue = 0;
	}
	if (Cnt_Point == 20)
	...
	if (CCDMain_Status.Left_LostFlag == 1 && CCDMain_Status.Right_LostFlag == 1)
	{
		...
		{
			Flag_Cross = 1;
			CCDMain_Status.MidLineSearchStart = 0;
		}
		for (i = 0; i < 20; i++)
		{
			if (LostFlagArr_Left[i] == 1)
			{
				j++;
			}
			if (LostFlagArr_Right[i] == 1)
			{
				k++;
			}
		}
	...
	else
	{
		...
		if (Flag_Cross == 1)
		{
			Dir_PID.ControlValue = Dir_PID.ControlValue*(0.2*Cnt_ExitCross);
			Cnt_ExitCross++;
			//BeepBeepBeep(100);
			if (Cnt_ExitCross >= 5)
			{
				CCDMain_Status.MidLineSearchStart = 1;
				Cnt_ExitCross = 0;
				Cnt_Cross = 0;
				Flag_Cross = 0;
			}
...
void CCDLineInit(void)
{
	...
	/*if (CarInfo_Now.CarSpeed>20)
	{
	LeftArr[Cnt_Point] = CCDMain_Status.LeftPoint;
	RightArr[Cnt_Point] = CCDMain_Status.RightPoint;
	Cnt_Point++;
	if (Cnt_Point==10)
	{
	for (i = 0; i < 10;i++)
	...
	CCDMain_Status.LeftSet = Sum1 / 10;
	CCDMain_Status.RightSet = Sum2 / 10;
	CCDMain_Status.MidSet = (CCDMain_Status.LeftSet + CCDMain_Status.RightSet) / 2;
	CCDMain_Status.SearchBegin = CCDMain_Status.MidSet;
	CCDMain_Status.LineWidth = (short)((CCDMain_Status.RightSet - CCDMain_Status.LeftSet)*0.7);
	BeepBeepBeep(1000);
