...
void MPU6050_Init(void)
{
  ...
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, MPU6050_PWR_MGMT_1, 0x80);
  LPLD_SYSTICK_DelayMs(1);
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, MPU6050_PWR_MGMT_1, 0x00);
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, 0x19, 0x04);
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, MPU6050_CONFIG, 0x02);//低通滤波器带宽5Hz
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, MPU6050_GYRO_CONFIG, 0x18);//不自检   ±500 °/s
  i2c_writeaddr(MPU6050_I2C_Moudle, MPU6050_ADDRESS, MPU6050_ACCEL_CONFIG, 0x00);//不自检  ±4g
}
