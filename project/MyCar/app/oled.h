...
//OLED显示屏
//定义输出
#define LED_SCLK(x) LPLD_GPIO_Output_b(PTC, 4, x)// 时钟定义 
#define LED_SDA(x)  LPLD_GPIO_Output_b(PTC, 5, x)//数据口D0
#define LED_RST(x)  LPLD_GPIO_Output_b(PTC, 6, x)//复位低能电平
#define LED_DC(x)  LPLD_GPIO_Output_b(PTC, 7, x)//偏置常低
//OLED显示函数
/*定义ASCII码表*/
