...
uint8 LPLD_Nrf24L01_Init(void)
{
 ...
 // spi_init_param.SPI_MosiPin = NRF24L01_MOSI;
  ...
  //配置CE
  ce_io_param.GPIO_PTx  = PTA;
  ce_io_param.GPIO_Pins = GPIO_Pin28;
  ce_io_param.GPIO_Dir  = DIR_OUTPUT;
  ce_io_param.GPIO_Output = OUTPUT_L;
  
  IRQ_io_param.GPIO_PTx=PTC;
  IRQ_io_param.GPIO_Pins=GPIO_Pin0;
  IRQ_io_param.GPIO_Dir=DIR_INPUT;
  LPLD_GPIO_Init(IRQ_io_param);
 ...
 *    无
 */
uint8 LPLD_Nrf24L01_RecvFrame(uint8 *data, uint8 len)
{
  uint8 Status;
 // NRF24L01_CE_H;
//  while(LPLD_GPIO_Input_b(PTC,0)==1);
 // NRF24L01_CE_L;
  
  
  Status = LPLD_Nrf24L01_ReadReg(NRF24L01_STATUS);
   LPLD_Nrf24L01_WriteReg(NRF24L01_WRITE_REG+NRF24L01_STATUS,Status); //清除中断
  if(Status&NRF24L01_STATUS_RX_DR)
  {
  LPLD_Nrf24L01_ReadBuf(NRF24L01_RD_RX_PLOAD,data,len);
  LPLD_Nrf24L01_WriteReg(NRF24L01_FLUSE_RX,NRF24L01_NOP);
  return 1;
  }
